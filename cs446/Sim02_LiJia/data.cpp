 ...
 *
 * @details output all the meta-data time and thread metrics to the monitor. 
 *          
               ...
               if( strcmp( metaDataStream[index].description, "allocate" ) == 0 )
         		{
                  ...
                  wait( t1, t2, cycles ); 
                  elapsedTime = findTime( t1, t2 );
                  memoryNum = allocateMemory( memoryNum ); 
               	cout << setprecision(6) << elapsedTime << " - " << "Process" << processID << ": allocating memory"<< endl;
               	gettimeofday(&t2, NULL); 
                  elapsedTime = findTime( t1, t2 ); 
               	cout << setprecision(6) << elapsedTime << " - " << "Process" << processID << ": memory allocated at Ox"<< memoryNum << endl;
         		}
 ...
 *
 * @details output all the meta-data time and thread metrics to a specific file location. 
 *          
 * @param in: metaDataStream, fileData, processObj
 *
 * @note None
               ...
               if( strcmp( metaDataStream[index].description, "allocate" ) == 0 )
         		{
                  ...
                  memoryNum = allocateMemory( memoryNum ); 
               	...
               	gettimeofday(&t2, NULL); 
                  elapsedTime = findTime( t1, t2 ); 
               	fout << setprecision(6) << elapsedTime << " - " << "Process" << processID << ": memory allocated at Ox"<< memoryNum << endl;
         		}
               else if( strcmp( metaDataStream[index].description, "block" ) == 0 )
         		{
                  processObj.processState = READY;
